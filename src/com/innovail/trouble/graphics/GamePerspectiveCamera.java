...
public class GamePerspectiveCamera extends PerspectiveCamera
{
    ...
    {
        final Vector2 overlayAngle = new Vector2 (_rotationAngle.x, _rotationAngle.y);
        return overlayAngle;
    }
    ...
    public Vector2 lookAtPosition (final Vector3 lookAtPoint, final Vector3 from)
    {
        ...
        final float sqr = normFrom.x * normFrom.x + normFrom.y * normFrom.y + normFrom.z * normFrom.z;
        _radius = (float) Math.sqrt (sqr);

        if (_rotationAngle == null) {
            _rotationAngle = new Vector2 ();
        }
        _rotationAngle.x = com.badlogic.gdx.math.MathUtils.radiansToDegrees * com.badlogic.gdx.math.MathUtils.atan2 (normFrom.x, normFrom.z);
        _rotationAngle.y = com.badlogic.gdx.math.MathUtils.radiansToDegrees * (float) Math.acos (normFrom.y / _radius);
    ...
    public void rotateAroundLookAtPoint (final Vector2 angleIncrease)
    {
        if (_radius != 0.0f) {
            ...
            if ((newAngle.x > _cutOffAngle.get (_MIN).x) && (newAngle.x < _cutOffAngle.get (_MAX).x)) {
            ...
            if ((newAngle.y > _cutOffAngle.get (_MIN).y) && (newAngle.y < _cutOffAngle.get (_MAX).y)) {
