...
public class MenuEntryCount extends MenuEntry
{
    ...
    public boolean handleIntersect (final Ray touchRay)
    {
        if (Intersector.intersectRayBoundsFast (touchRay,
                                                _manipulators.get (SmallerKey).getBoundingBox ())) {
            if (_DEBUG) {
            ...
            return true;
                                                       ...
                                                       _manipulators.get (LargerKey).getBoundingBox ())) {
            if (_DEBUG) {
    ...
    private void initialize ()
    {
        ...
        _manipulators.put (SmallerKey,
                           new GameMesh (ApplicationSettings.getInstance ().getGameFont (GameFont.typeToString (FontType.MESH)).getMeshFont ().createStillModel (SmallerKey)));
        _manipulators.put (LargerKey,
                           new GameMesh (ApplicationSettings.getInstance ().getGameFont (GameFont.typeToString (FontType.MESH)).getMeshFont ().createStillModel (LargerKey)));
    }

    @Override
    public void processParams (final Parameters params)
    {
        setMinMaxCount (params.getIntParameter ("mincount"),
                        params.getIntParameter ("maxcount"));
    }
                        ...
                        final Color color)
    {
        ...
        if ((_manipulators != null) && !_manipulators.isEmpty ()) {
            if (_minCount != _maxCount) {
                render (gl, menuOffset, _manipulators.get (SmallerKey),
                        _manipPosition.get (SmallerKey), color);
            }
            render (gl, menuOffset,
                    _manipulators.get (Integer.toString (_currentCount)),
                    _manipPosition.get (Integer.toString (_currentCount)),
                    color);
            if (_minCount != _maxCount) {
                render (gl, menuOffset, _manipulators.get (LargerKey),
                        _manipPosition.get (LargerKey), color);
            }
        }
    }
        ...
        if (_manipulators != null) {
            for (int i = _minCount; i <= _maxCount; i++) {
                if (!_manipulators.containsKey (Integer.toString (i))) {
                    _manipulators.put (Integer.toString (i),
                                       new GameMesh (ApplicationSettings.getInstance ().getGameFont (GameFont.typeToString (FontType.MESH)).getMeshFont ().createStillModel (Integer.toString (i))));
                }
            }
        }
    }
    ...
    public void setOffset (final Vector3 offsetPosition)
    {
        ...
        _manipPosition.put (LargerKey,
                            new Vector3 (-offsetPosition.x + totalX, offsetPosition.y, offsetPosition.z));

        totalX -= ManipulatorGap;
        final int countSize = (_maxCount - _minCount) + 1;
        float maxSizeX = 0.0f;
        final float [] xSize = new float [countSize];
        for (int i = _minCount, j = 0; i <= _maxCount; i++, j++) {
        ...
        for (int i = _minCount, j = 0; i <= _maxCount; i++, j++) {
            final String currentCount = Integer.toString (i);
            final float actualX = (maxSizeX / 2) + (xSize[j] / 2);
            _manipPosition.put (currentCount,
        ...
        tDim = tBB.getDimensions ();
        totalX -= (ManipulatorGap + (tDim.x / 2));
        _manipPosition.put (SmallerKey,
                            new Vector3 (-offsetPosition.x + totalX, offsetPosition.y, offsetPosition.z));
    }
}
